...
# Inside the KalmanFilterGUI class

    def initUI(self):
        self.setWindowTitle('Kalman Filter GUI')
        self.setGeometry(100, 100, 1200, 800)  # Adjusted height for better fit
        self.setStyleSheet("""
            QWidget {
            ...
            QComboBox QAbstractItemView {
                background-color: #2A2A2A;
                selection-background-color: #3A9;
                color: white;
            }
        ...
        self.range_time_button.clicked.connect(lambda: self.plot_type_selected("Range vs Time"))
        visualization_layout.addWidget(self.range_time_button)
        self.azimuth_time_button = QPushButton("Azimuth vs Time")
        self.azimuth_time_button.clicked.connect(lambda: self.plot_type_selected("Azimuth vs Time"))
        visualization_layout.addWidget(self.azimuth_time_button)
        self.elevation_time_button = QPushButton("Elevation vs Time")
        self.elevation_time_button.clicked.connect(lambda: self.plot_type_selected("Elevation vs Time"))
        ...
        self.select_all_radio.toggled.connect(self.toggle_select_all_tracks)
        ...
        plot_settings_layout.addWidget(self.select_all_checkbox)

        # Add scroll area for track checkboxes
        self.track_scroll_area = QScrollArea()
        self.track_scroll_area.setWidgetResizable(True)
        self.track_scroll_area.setFixedHeight(150)  # Set a fixed height for the scroll area

        # Container widget for checkboxes
        self.track_checkbox_container = QWidget()
        self.track_selection_layout_inner = QVBoxLayout(self.track_checkbox_container)
        self.track_scroll_area.setWidget(self.track_checkbox_container)
        plot_settings_layout.addWidget(self.track_scroll_area)
    ...
    def plot_type_selected(self, plot_type):
        self.current_plot_type = plot_type
        self.update_plot()

    def toggle_select_all_tracks(self):
        if self.select_all_checkbox.isChecked():
            self.selected_track_ids = set(track['track_id'] for track in self.tracks)
    ...
    def update_selected_tracks(self):
        self.selected_track_ids = set(
            track['track_id'] for track, checkbox in zip(self.tracks, self.track_checkboxes) if checkbox.isChecked()
        )
        self.update_plot()
    ...
    def update_track_checkboxes(self):
        ...
        for checkbox in self.track_checkboxes:
            self.track_selection_layout_inner.removeWidget(checkbox)
